...
// this secret server-side) may call it now.
export function requireBotSecret(req: Request, res: Response, next: NextFunction): void {
  ...
  const incoming = req.headers['x-bot-token'];
  if (!incoming || incoming !== secret) {
    console.warn(`[visitors] Invalid or missing X-Bot-Token on ${req.method} ${req.path}`);
